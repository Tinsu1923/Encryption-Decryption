...
int main(){
    backtofirst:
    int array[13]; // we are creating an array of 13 integers, most of which are used to store digits.  
    char letter;   
    do{
   ...
   std::cin>>letter;
    ...
    if(letter == 'Y' || letter == 'y') // if user wants to encrypt.
    {
       ...
       array[9] = array[7];
       array[10] = array[8];
        // we are displaying the number in four digit form.
       std::cout<<"The encrypted number is "<<array[9]<<array[10]<<array[11]<<array[12]<<std::endl;
    }
    else if (letter =='N' || letter =='n') //if user wants to decrypt instead.
    {
       
        // we extract the individual digits from the number we accepted, in the next 4 lines of code.
       array[1] = (array[0] / 1000) % 10;
       array[2] = (array[0] / 100) % 10;
       array[3] = (array[0] / 10) % 10;
       array[4] = array[0] % 10;
       ...
       array[9] = array[7];
       array[10] = array[8]; 
       std::cout<<"The original number is "<<array[9]<<array[10]<<array[11]<<array[12]<<std::endl;
    }
    std::cout<<"Do you want to continue? "<<std::endl;
    std::cout<<"Type Y/y for yes or N/n for no: ";
    std::cin>>letter;
    if (letter == 'Y' || letter == 'y') // repeat the process.
    goto backtofirst;
